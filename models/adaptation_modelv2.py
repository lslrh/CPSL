...
class CustomModel():
    ...
    def step(self, source_x, source_label, target_x, queue, queue_label, queue_out, use_the_queue, use_the_queue_out, target_imageS=None, target_params=None, target_lp=None, 
        ...
        if cutmix<0.5:
#             positions = rand_bbox(mask_shape=(source_x.shape[0], source_x.shape[1]))
#             bbx1, bby1, bbx2, bby2 = np.int16(positions)
        ...
        # out_SL_DS = F.softmax(out_SL_DS, dim=2)
...
#                 self.update_objective_SingleVector(ema_ids[t], ema_vectors[t].detach(), start_mean=False)
        ...
        return loss.item(), loss_CTS.item(), loss_consist.item(), loss_SL.item(), loss_contrast, queue, queue_label, queue_out
